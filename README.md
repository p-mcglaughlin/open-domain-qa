...
The [wiki](https://github.com/p-mcglaughlin/open-domain-qa/wiki) provides a more detailed discussion these topics.
...
This project uses [OpenSearch](https://opensearch.org/) to implement BM25 based full-text search. Each Wikipedia article is split into paragraphs, and indexed using the [English analyzer](https://docs.opensearch.org/docs/latest/analyzers/language-analyzers/english/) which removes stop words and performs stemming. Stemming algorithms reduce words to their common base, e.g., bake, baker, baking, baked -> bake. This process groups words with similar meanings, and typically improves search results. User queries are not stemmed since it produces unusual results, see [TF-IDF wiki](https://github.com/p-mcglaughlin/open-domain-qa/wiki/TF%E2%80%90IDF_BM25) for more details and examples.

## Embedding Models
TF-IDF provides an intuitive and interpretable approach to IR. However, these methods do not 'understand' the user's query, see [TF-IDF wiki](https://github.com/p-mcglaughlin/open-domain-qa/wiki/TF%E2%80%90IDF_BM25#Disadvantages) a detailed discussion. Embedding based approaches aim to address this issue by using machine learning models to encode a text's semantic information into a vector (essentially a list of numbers). We can imagine these vectors (or embeddings) as points in some abstract space where similar passages of text are 'close' to each other. To use embeddings for IR, we compute the query's embedding and search for the closests points, see the illustration below. Algorithms for nearest neighbor search are outlined [here](wiki/Approximate-Nearest-Neighbors).
